...
def test_text_tokens_none_when_no_additional_information():
    assert _request_tts_text_tokens(_Req(None)) is None
...
def _req_with_precomputed_ids(rid, n_ids):
    entry = SimpleNamespace(list_data=[list(range(n_ids))])
    ai = SimpleNamespace(entries={PRECOMPUTED_TEXT_IDS_KEY: entry})
...
def test_async_chunk_forwards_segment_text_tokens_in_payload_meta():
    """The transferred chunk payload carries n_k in meta.segment_text_tokens.

    The producer writes the request's precomputed text-id count into the
    payload meta, so Code2Wav can compute rho instead of logging n/a.
    """
    rid = "nk-rid"
    ...
    payload = talker2code2wav_async_chunk(
        ...
        request=_req_with_precomputed_ids(rid, n_ids=12),
        is_finished=False,
    )
    assert payload is not None
    assert payload.meta.segment_text_tokens == 12
